...
# data fetching
diseases = ['健康 (Healthy)', '氣喘 (Asthma)', '支氣管擴張 (Bronchiectasis)', '細支氣管炎 (Bronchiolitis)', '慢性阻塞性肺病 (COPD)', '下呼吸道感染 (LRTI)', '肺炎 (Pneumonia)', '上呼吸道感染 (URTI)']
positions = ['主氣管', '左肺前側', '右肺前側', '左肺後側', '右肺後側', '左肺側面', '右肺側面']
mfcc_len = 864
...
print('檢測結果 Inspection Report')
print('-' * 20)
print('呼吸音檢測 Respiratory Sound Inspection', end = '\n\n')
no_noise = True
for i in range(7):
...
if no_noise:
    ...
    print('所錄得之呼吸音異常，建議進一步至醫療機構諮詢相關醫療人員。')
print('-' * 20)
print('呼吸疾病推測 Respiratory Disease Prediction', end = '\n\n')
for i in range(8):
    print('{:}'.format(report[i][1]) + ': {:>.3%}'.format(float(report[i][0])))
print()
if report[0][0] < 0.5:
    print('注意：您的錄音中沒有任何疾病或狀態推估機率超過50%，建議重新錄音檢測或是尋求醫療人員諮詢。')
elif report[0][1] != diseases[0]:
    print('注意：您可能有潛在的呼吸疾病「' + '{}'.format(report[0][1]) + '」，建議進一步至醫療機構諮詢相關醫療人員。')
print('-' * 20)
